function [reconstructed] = ReconstructorGPU(image,ref_wave,template_wave)

% rval = zslice;
Sn_pixels = 3000;
OVS = 2;
n_pixels = 6000;
    image = single(image(1:Sn_pixels,1:Sn_pixels));
    image = single(imresize(image,OVS));
    down_sz = n_pixels/OVS; %this is what sensor sees
    I2_down = imresize(image - abs(ref_wave).^2, [down_sz, down_sz], 'cubic'); %find intensity then remove its effect

    I2_down_fft = fft2(I2_down); %of data
    I2_up_fft   = single(zeros(n_pixels, n_pixels)); %insert zeros
    ...
    despread = I2_up.*ref_wave; % 'despreading' the signal
    clear I2_up;
    clear I2_up_fft;

    [X,Y] = meshgrid(single(linspace(-1,1,n_pixels)), single(linspace(-1,1,n_pixels)));
