...
#include "Reconstructor_Kanka_Mod.cpp"
#include "Reconstruct_tile.cpp"
#endif
...
{       
    ...
    if(z_slices[0] == -1){
        ...
        // vector<int> slices = {-500, -450, -400, -350, -300, -250, -200, -150, -100, -50, 0, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500};
        vector<int> slices = {-160, -155, -150, -145, -140, -135, -130, -125, -120, -115, -110, -105, -100, -95, -90, -85, -80, -75, -70, -65, -60, -55, -50, -45, -40, -35, -30, -25, -20, -15, -10, -5, 0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60};
        // vector<int> slices = {0};

        for(int iz = 0; iz < slices.size(); iz++)
        {
            cout << "z-slice " << slices[iz] << endl;

            // Mat recon = Reconstructor_Kanka(diffStack, slices);
            Mat recon = Reconstructor(diffStack, slices[iz]);
            // Mat recon = Reconstructor_Kanka(diffStack, slice);

            // if(verbose) cout << "Reconstructed z-slice " << slice + 501 << ", saving..." << endl << endl;

            string name = "detect_recon/" + sampleID + "_z-slice_" + to_string(slices[iz] + 500); // set the name of the output image
